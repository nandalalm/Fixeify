...
export const EditCategory: FC<EditCategoryProps> = ({ onClose, onSuccess, category }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data: { name?: string; image?: string } = { name: formattedName };
      ...
      if (errorResponse.response?.data?.message === "Category name already exists") {
        ...
        setErrors({ general: errorResponse.response?.data?.message || "Failed to update category" });
      }
    } finally {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    if (nameError) {
      setErrors({ name: nameError, image: undefined });
      return;
    }
  ...
  return (
    <div className="bg-white rounded-lg p-6 w-full max-w-md mx-auto shadow-lg relative">
      ...
      <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800">Edit Category</h2>
      <form onSubmit={handleFormSubmit}>
        ...
        <div className="mb-6">
          ...
          <div
            ...
            onDrop={handleDrop}
          >
            <h3 className="text-lg font-medium text-gray-700">Update Category Image</h3>
            {isUploadingImage ? (
          ...
          {image && (
            <div className="mt-4 flex justify-center">
              <img src={image} alt={category.name} className="w-16 h-16 object-cover rounded-full border border-gray-200" />
            </div>
          )}
